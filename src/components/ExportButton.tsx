// filepath: c:\Users\josh_\git\agents\src\components\ExportButton.tsx
'use client';
  ...
  const handleExport = () => {
    ...
    try {
      ...
      const title = workflowState.topic 
        ...
        : 'Semantic Kernel Agents Conversation';
      ...
      a.href = url;
      a.download = `${title.replace(/[^a-z0-9]/gi, '-').toLowerCase()}-${new Date().toISOString().split('T')[0]}.md`;
      a.click();
      ...
      alert('There was an error exporting your conversation. Please try again.');
    } finally {
