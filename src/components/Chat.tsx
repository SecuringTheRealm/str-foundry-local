...
const defaultAgents: Agent[] = [
  {
    ...
    systemPrompt: 'You are a helpful Concierge that asks users what they want to learn about.',
    isActive: true
  },
  {
    id: 'researcher',
    ...
    systemPrompt: 'You are an expert researcher who provides comprehensive information on topics.',
    isActive: false
  },
  {
    id: 'copywriter',
    ...
    systemPrompt: 'You are a skilled copywriter who transforms research into engaging content.',
    isActive: false
  },
  {
    id: 'reviewer',
    ...
    systemPrompt: 'You are a detail-oriented reviewer who provides constructive feedback on content.',
    isActive: false
  }
];
...
export default function Chat() {
  ...
  const getActiveAgentId = (): string => {
    switch (workflowState.stage) {
      case 'inquiry': return 'concierge';
      case 'research': return 'researcher';
      case 'writing': return 'copywriter';
  ...
  useEffect(() => {
    ...
    if (messages.length === 0) {
      const welcomeMessage: Message = {
        id: uuidv4(),
        role: 'concierge',
        content: 'Hello! I\'m your concierge. What topic would you like me to help you learn about today?',
        timestamp: new Date()
      };
  ...
  useEffect(() => {
    const handleWorkflowTransition = async () => {
      ...
      if (lastMessage.role === 'user' || lastMessage.content.includes("I'll research") || 
      ...
      switch (workflowState.stage) {
        ...
        case 'writing':
          ...
          if (lastMessage.role !== 'copywriter' && workflowState.researchNotes) {
            transitionMessage = {
              id: uuidv4(),
              role: 'copywriter',
      ...
      if ((workflowState.stage === 'research' || workflowState.stage === 'writing') && transitionMessage) {
        setIsLoading(true);
        setTimeout(async () => {
          try {
            const response = await fetch('/api/chat', {
              ...
              body: JSON.stringify({
                message: workflowState.stage === 'research' 
                  ? `Please research ${workflowState.topic}` 
                  : `Please write content based on these notes: ${workflowState.researchNotes?.substring(0, 500)}...`,
                agentId: workflowState.stage === 'research' ? 'researcher' : 'copywriter',
                systemPrompt: agents.find(a => a.id === (workflowState.stage === 'research' ? 'researcher' : 'copywriter'))?.systemPrompt,
                history: messages,
                workflowState
              })
            });
            ...
            if (response.ok) {
              const currentAgentRole = workflowState.stage === 'research' ? 'researcher' : 'copywriter' as any;
              ...
              if (data.updatedState && Object.keys(data.updatedState).length > 0) {
                setWorkflowState(prevState => ({ ...prevState, ...data.updatedState }));
              }
            }
          } catch (error) {
  ...
  const getAgentRoleForStage = (stage: string): 'concierge' | 'researcher' | 'copywriter' | 'reviewer' | 'agent' => {
    switch (stage) {
      case 'inquiry': return 'concierge';
      case 'research': return 'researcher';
      case 'writing': return 'copywriter';
