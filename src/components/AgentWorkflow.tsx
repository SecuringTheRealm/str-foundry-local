// filepath: c:\Users\josh_\git\agents\src\components\AgentWorkflow.tsx
'use client';
...
const AgentWorkflow = ({ agents, workflowState, onStageComplete }: AgentWorkflowProps) => {
  ...
  const getProgressPercentage = (): number => {
    const stages: WorkflowStage[] = ['inquiry', 'research', 'writing', 'review', 'complete'];
  ...
  return (
    <div className="flex flex-col gap-3 p-4 bg-white rounded-lg shadow">
      <h3 className="text-lg font-semibold text-gray-800">Multi-Agent Workflow</h3>
      ...
      <div className="flex items-center gap-2 mt-1">
        ...
        <div>
          <div className="text-sm font-medium text-gray-900">Active: {getActiveAgentName()}</div>
          <div className="text-xs text-gray-500">{getStageDescription()}</div>
        </div>
      </div>
    </div>
  );
