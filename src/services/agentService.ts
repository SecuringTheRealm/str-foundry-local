...
Be thorough but respectful, acknowledging the strengths of the work while proposing enhancements.`
  ...
  async processMessage(request: ChatRequest): Promise<string> {
    ...
    if (!process.env.AZURE_OPENAI_API_KEY) {
      return 'API key is missing. Please add your Azure OpenAI API key to the environment variables.';
    }
    ...
    try {
      ...
      const completion = await openai.chat.completions.create({
        model: process.env.AZURE_OPENAI_DEPLOYMENT || 'gpt-4o', // Use the deployment name as the model
